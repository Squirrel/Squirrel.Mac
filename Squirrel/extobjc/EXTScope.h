...
#define onExit \
    try {} @finally {} \
    __strong sqrl_cleanupBlock_t metamacro_concat(sqrl_exitBlock_, __LINE__) __attribute__((cleanup(sqrl_executeCleanupBlock), unused)) = ^
...
#define weakify(...) \
    try {} @finally {} \
    metamacro_foreach_cxt(sqrl_weakify_,, __weak, __VA_ARGS__)
...
#define unsafeify(...) \
    try {} @finally {} \
    metamacro_foreach_cxt(sqrl_weakify_,, __unsafe_unretained, __VA_ARGS__)
...
#define strongify(...) \
    ...
    metamacro_foreach(sqrl_strongify_,, __VA_ARGS__) \
    _Pragma("clang diagnostic pop")

/*** implementation details follow ***/
typedef void (^sqrl_cleanupBlock_t)();

void sqrl_executeCleanupBlock (__strong sqrl_cleanupBlock_t *block);

#define sqrl_weakify_(INDEX, CONTEXT, VAR) \
    CONTEXT __typeof__(VAR) metamacro_concat(VAR, _weak_) = (VAR);

#define sqrl_strongify_(INDEX, VAR) \
